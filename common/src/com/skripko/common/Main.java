...
//		debug("isProxyWorks(): " + isProxyWorks$Refresh());
		...
		List<String> choosedStates = statesOption.stream().filter(
				state -> state.contains("Missi")).collect(Collectors.toList());
		...
		for (String choosedState : choosedStates) {
			...
			while (true) {
				...
				schools.addAll(searchResultTitles.stream().map(schoolListRow -> {
					...
					bean.affiliation = openNewTabAndMap(schoolNameEl, in -> {
				...
				SelenideElement element = getElement(By.xpath("//a[@class='ignoredclass1' and contains(., 'Next >>')]"));
				if (!element.exists()) {
					break;
				}
				element.click();
